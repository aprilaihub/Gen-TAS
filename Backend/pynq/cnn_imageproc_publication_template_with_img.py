...
def load_fashionmnist_test(data_zip: str | Path = "FashionMNIST_data.zip") -> tuple[np.ndarray, np.ndarray]:
    base = _base_dir()
    zip_path = next(
        (
            p
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            for dataset in ("FashionMNIST", "MNIST"):
                ...
                if img in names and lbl in names:
                    return _read_idx_images_from_bytes(zf.read(img)), _read_idx_labels_from_bytes(zf.read(lbl))
    ...
    for dataset in ("FashionMNIST", "MNIST"):
        ...
        if img_path.is_file() and lbl_path.is_file():
            return _read_idx_images_from_bytes(img_path.read_bytes()), _read_idx_labels_from_bytes(lbl_path.read_bytes())
    raise FileNotFoundError("Could not find FashionMNIST_data.zip/data_mnist.zip or extracted IDX test files.")


def quantize_fashionmnist_image(image_u8: np.ndarray) -> np.ndarray:
    return quantize_data_t(image_u8.reshape(-1).astype(np.float32) / 255.0)


def class_name(label: int) -> str:
    if 0 <= int(label) < len(FASHIONMNIST_CLASS_NAMES):
        return FASHIONMNIST_CLASS_NAMES[int(label)]
    return f"unknown({label})"


def print_class_mapping() -> None:
    print("\nFashion-MNIST class mapping:")
    for index, name in enumerate(FASHIONMNIST_CLASS_NAMES):
        print(f"  {index}: {name}")
...
def measure_accuracy(ip, a, b, images, labels, weights, num_samples: int, warmup: int, progress_every: int):
    ...
    warm_input = quantize_fashionmnist_image(images[0])
    ...
    for i in range(num_samples):
        input_q = quantize_fashionmnist_image(images[i])
        ...
        row = {
            ...
            "prediction": pred,
            "prediction_name": class_name(pred),
            "correct": is_correct,
...
def save_results(output_dir: Path, correct: int, confusion: np.ndarray, timings: dict, rows: list[dict], num_samples: int, args, verification: dict, cpu_baseline: dict | None) -> dict:
    ...
    summary = {
        ...
        "class_mapping": {str(index): name for index, name in enumerate(FASHIONMNIST_CLASS_NAMES)},
...
def print_results(correct, confusion, first16, timings, num_samples, summary: dict | None, print_logits: bool):
    ...
    print("Dataset      : FashionMNIST test")
    ...
    for i, label, pred, logits in first16:
        ...
        label_text = class_name(label)
        pred_text = class_name(pred)
        if print_logits:
            print(
                f"  Sample {i:2d}: expected={label_text}, predicted={pred_text}, {ok}, "
                f"logits={[float(x) for x in logits]}"
            )
        else:
            print(f"  Sample {i:2d}: expected={label_text}, predicted={pred_text}, {ok}")
    print("\nLatency summary:")
...
def display_random_test_images(
    images,
    labels,
    rows: list[dict],
    output_dir: Path,
    ...
    """Display random evaluated test images in Jupyter with correctness-colored labels."""
    if sample_count <= 0 or not rows:
        return
    try:
        import matplotlib.pyplot as plt
    except ImportError:
        print("matplotlib is not available; skipping random image preview.")
        return
    ...
    fig, axes = plt.subplots(rows_count, cols, figsize=(2.65 * cols, 3.55 * rows_count))
    ...
    for ax, row_index in zip(axes_flat, selected):
        row = rows[int(row_index)]
        sample_index = int(row["sample_index"])
        label = int(labels[sample_index])
        pred = int(row["prediction"])
        ok = label == pred
        ax.imshow(images[sample_index], cmap="gray", vmin=0, vmax=255)
        ax.set_title(
            f"test #{sample_index}\ntruth: {class_name(label)}\npred: {class_name(pred)}",
    ...
    fig.text(0.495, 0.945, "green = match", ha="right", va="top", fontsize=15, color="green", fontweight="bold")
    fig.text(0.505, 0.945, "red = mismatch", ha="left", va="top", fontsize=15, color="red", fontweight="bold")
    fig.subplots_adjust(top=0.78, hspace=0.52, wspace=0.08)

    output_dir.mkdir(parents=True, exist_ok=True)
    preview_path = output_dir / "random_test_samples.png"
    fig.savefig(preview_path, dpi=160, bbox_inches="tight", facecolor=fig.get_facecolor())
    print(f"Saved random image preview: {preview_path}")
    plt.show()
...
def parse_args():
    ...
    parser.add_argument("--preview-seed", type=int, default=None)
...
def main():
    ...
    images, labels = load_fashionmnist_test(args.data_zip)
    ...
    print(f"Running {num_samples} FashionMNIST samples for {PARTITION_ID}...")
    ...
    if not args.no_save:
        summary = save_results(
            ...
            verification, cpu_baseline
        )
    print_results(correct, confusion, first16, timings, num_samples, summary, args.print_logits)
    display_random_test_images(images, labels, rows, Path(args.output_dir), args.preview_samples, args.preview_seed)
