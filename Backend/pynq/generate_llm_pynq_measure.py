...
DEFAULT_CNN_PUBLICATION_TEMPLATE = Path(__file__).with_name("cnn_imageproc_publication_template.py")
DEFAULT_CAMC_PUBLICATION_TEMPLATE = Path(__file__).with_name("camc_publication_template.py")
...
def build_prompt(args: argparse.Namespace) -> tuple[str, str]:
    ...
    elif _is_camc_handoff(handoff):
        assert handoff is not None
        rendered = render_camc_publication_script(args, handoff)
        sections.append(artifact_section("DRAFT_CAMC_PUBLICATION_SCRIPT", args.handoff, rendered))
...
12. For CNNImageProc partitions, keep the source, weights, dataset, printed dataset name, and saved summary metadata consistent. If FashionMNIST_data.zip or cnn_imageproc_fashion/fmnist evidence is supplied, default to FashionMNIST_data.zip, load data/FashionMNIST/raw first, and report "FashionMNIST test". Use data_mnist.zip/data/MNIST/raw only for older MNIST artifacts. Never combine MNIST/v2 source evidence with FashionMNIST metadata or FashionMNIST weights unless the supplied handoff explicitly identifies a FashionMNIST workload.
13. Run accuracy, save per-sample predictions/timings CSV, save a confusion matrix CSV, save a summary JSON, and print a publication-friendly latency/accuracy summary.
14. Add concise comments identifying any design assumptions that remain unavoidable.
...
def _is_camc_handoff(handoff: dict[str, object] | None) -> bool:
    return bool(handoff) and handoff.get("workload") == "camc"
...
def _replace_generated_assignment(code: str, name: str, value: object) -> str:
    """Replace a generated assignment using its AST span, including multiline literals."""
    try:
        tree = ast.parse(code)
...
def render_cnn_publication_script(args: argparse.Namespace, handoff: dict[str, object]) -> str:
    ...
    generated_note = (
        "# Auto-generated from the generic CNNImageProc publication accuracy/latency runner.\n"
    )
    if code.startswith("#!"):
...
def render_camc_publication_script(args: argparse.Namespace, handoff: dict[str, object]) -> str:
    """Render the deterministic CAMC correctness and latency runner."""
    if not DEFAULT_CAMC_PUBLICATION_TEMPLATE.is_file():
        raise GenerationError(f"CAMC publication template not found: {DEFAULT_CAMC_PUBLICATION_TEMPLATE}")
    code = DEFAULT_CAMC_PUBLICATION_TEMPLATE.read_text(encoding="utf-8")
    vivado = handoff.get("vivado") if isinstance(handoff.get("vivado"), dict) else {}
    boundary = handoff.get("hardware_boundary") if isinstance(handoff.get("hardware_boundary"), dict) else {}
    hw_input = boundary.get("input") if isinstance(boundary.get("input"), dict) else {}
    hw_output = boundary.get("output") if isinstance(boundary.get("output"), dict) else {}
    ...
    if metrics_path and Path(str(metrics_path)).is_file():
        static_metrics = json.loads(Path(str(metrics_path)).read_text(encoding="utf-8"))
        static_metrics.setdefault("implementation", {})["pynq_script_generation"] = "passed"
    replacements = {
        "DESIGN_NAME": str(handoff.get("design_name") or Path(args.bitstream).with_suffix("").name),
        "BITSTREAM": Path(args.bitstream).name,
        "IP_NAME": str(vivado.get("ip_instance") or "camc_top_0"),
        "PARTITION_ID": handoff.get("partition_id") or "CAMC_PARTITION",
        "FPGA_STAGES": handoff.get("fpga_subfunctions") or [],
        "GPP_STAGES": handoff.get("gpp_subfunctions") or [],
        "HW_INPUT_LENGTH": int(hw_input.get("length") or 0),
        "HW_OUTPUT_LENGTH": int(hw_output.get("length") or 0),
    ...
    if not replacements["FPGA_STAGES"] or replacements["HW_INPUT_LENGTH"] <= 0 or replacements["HW_OUTPUT_LENGTH"] <= 0:
        raise GenerationError("CAMC handoff is missing a hardware stage or boundary length")
    for name, value in replacements.items():
        code = _replace_literal(code, name, value)
    code = _replace_assignment(code, "ITERATIONS", args.iterations)
    code = _replace_assignment(code, "WARMUP", args.warmup)
    code = _replace_assignment(code, "KERNEL_TIMEOUT_SECONDS", float(args.kernel_timeout))
    return code
...
def record_pynq_llm_usage(
    ...
    metrics_path = Path(handoff["experiment_metrics_path"])
    current = json.loads(metrics_path.read_text(encoding="utf-8")) if metrics_path.is_file() else {}
    prior_stage = (
        current.get("llm", {}).get("stages", {}).get("pynq_script_generation", {})
    )
    prior_usage = prior_stage.get("token_usage", {}) if isinstance(prior_stage, dict) else {}

    if status == "response_received":
        current_usage = dict(usage or {})
        current_usage["attempt_count"] = 1
        current_usage["finish_reasons"] = [current_usage.get("finish_reason")]
        current_usage["truncated_attempts"] = [1] if current_usage.get("truncated") else []
        if prior_stage.get("status") == "validation_failed" and prior_usage:
            prior_attempts = int(prior_usage.get("attempt_count") or 1)
            combined = dict(current_usage)
            for field in (
                "input_tokens",
                "output_tokens",
                "total_tokens",
                ...
                values = (prior_usage.get(field), current_usage.get(field))
                combined[field] = sum(value for value in values if isinstance(value, int))
            prior_parameters = prior_usage.get("request_parameters")
            if not isinstance(prior_parameters, list):
                prior_parameters = [prior_parameters] if prior_parameters else []
            combined["request_parameters"] = prior_parameters + [
                current_usage.get("request_parameters")
            ]
            combined["attempt_count"] = prior_attempts + 1
            combined["finish_reasons"] = list(
                prior_usage.get("finish_reasons") or [prior_usage.get("finish_reason")]
            ) + [current_usage.get("finish_reason")]
            combined["truncated_attempts"] = list(
                prior_usage.get("truncated_attempts") or []
            ) + ([prior_attempts + 1] if current_usage.get("truncated") else [])
            usage = combined
            token_count = int(combined.get("total_tokens") or token_count)
            elapsed_seconds = float(prior_stage.get("generation_time_s") or 0.0) + elapsed_seconds
        else:
            usage = current_usage
    elif status not in {"deterministic", "dry_run", "not_used"} and (
        prior_stage.get("status") == "response_received"
        or (prior_stage and not usage and not token_count)
    ):
        usage = prior_usage
        token_count = int(prior_stage.get("token_count") or token_count)
        elapsed_seconds = float(prior_stage.get("generation_time_s") or elapsed_seconds)

    model = None if status in {"deterministic", "dry_run", "not_used"} else args.model
    record_llm_usage(
        metrics_path,
...
def generate(args: argparse.Namespace, llm_client=None) -> dict[str, object]:
    ...
    publication_handoff = _is_cnn_imageproc_handoff(handoff) or _is_camc_handoff(handoff)
    if publication_handoff and args.generation_mode == "deterministic":
        ...
        if _is_cnn_imageproc_handoff(handoff):
            ...
            template = "cnn_imageproc_publication_deterministic"
        else:
            code = render_camc_publication_script(args, handoff)
            template = "camc_publication_deterministic"
        validate_generated_python(code, args.bitstream)
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(code, encoding="utf-8")
        if handoff.get("experiment_metrics_path"):
            from Evaluation.CNNImageProc.AI.experiment_metrics import update_metrics
            update_metrics(
                handoff["experiment_metrics_path"],
                {
        ...
        return {
            ...
            "elapsed_seconds": 0.0,
            "template": template,
            "generation_mode_requested": "deterministic",
            "generation_mode_used": "deterministic",
            "fallback_used": False,
            "fallback_reason": None,
        }

    if args.response_input:
        response = args.response_input.read_text(encoding="utf-8")
        code = refresh_embedded_metrics(extract_python(response), handoff)
        validate_generated_python(code, args.bitstream)
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(code, encoding="utf-8")
        if isinstance(handoff, dict) and handoff.get("experiment_metrics_path"):
            project_root_text = str(PROJECT_ROOT)
            if project_root_text not in sys.path:
                sys.path.insert(0, project_root_text)
            from Evaluation.CNNImageProc.AI.experiment_metrics import update_metrics
            ...
            prior_stage = (
                existing_metrics.get("llm", {})
                .get("stages", {})
                .get("pynq_script_generation", {})
            )
            provider_generated = bool(prior_stage.get("token_count"))
            update_metrics(
                metrics_path,
                {
                    "implementation": {"pynq_script_generation": "passed"},
                    "llm": {
                        "stages": {
                            "pynq_script_generation": {
                                "status": "passed" if provider_generated else "passed_revalidated_response",
                                "uses_llm_api": True,
                            }
                        }
                    },
                    "generation": {
                        "pynq": {
                            "mode_requested": args.generation_mode,
                            "mode_used": "llm" if provider_generated else "llm_revalidated_response",
                            "model": args.model,
                            "validation_passed": True,
                            "fallback_used": False,
                            "fallback_reason": None,
                            "response_input": None,
                            "response_revalidation": {
                                "status": "passed",
                                "response_input": str(args.response_input),
                                "uses_new_llm_call": False,
                            },
                        }
                    },
                },
        ...
        return {
            ...
            "dry_run": False,
            "prompt_output": prompt_output,
            "response_output": args.response_input,
            "output": args.output,
            "token_count": 0,
            "elapsed_seconds": 0.0,
            "generation_mode_requested": args.generation_mode,
            "generation_mode_used": "llm_revalidated_response",
            "fallback_used": False,
            "fallback_reason": None,
            "template": "publication_llm_revalidated_response",
        }
    ...
    try:
        ...
        code = (
            render_cnn_publication_script(args, handoff)
            if _is_cnn_imageproc_handoff(handoff)
            else render_camc_publication_script(args, handoff)
        )
        validate_generated_python(code, args.bitstream)
    ...
    if _is_cnn_imageproc_handoff(handoff):
        result["template"] = (
            "cnn_imageproc_publication_deterministic_fallback"
            if fallback_reason
            else "cnn_imageproc_publication_llm_assisted"
        )
    elif _is_camc_handoff(handoff):
        result["template"] = (
            "camc_publication_deterministic_fallback"
            if fallback_reason
            else "camc_publication_llm_assisted"
        )
    return result
